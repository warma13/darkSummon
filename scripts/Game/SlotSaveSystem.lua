...
function SlotSaveSystem.Update(dt)
    ...
    -- 加载超时保护：渐进式重试（5s → 6s → 7s → ... → 10s），达到上限后本地缓存兜底
    if loadInProgress and loadStartTime > 0 then
        if os.time() - loadStartTime >= loadCurrentTimeout then
            ...
            if loadCurrentTimeout < LOAD_TIMEOUT_MAX then
                ...
                print("[SlotSave] Retrying load slot " .. loadSlotId .. " with timeout " .. loadCurrentTimeout .. "s")
                loadInProgress = false
                local cb = loadCallback
                loadCallback = nil
                SlotSaveSystem.LoadSlot(loadSlotId, function(success, isNewSlot)
                    if cb then cb(success, isNewSlot) end
                end)
            else
                -- 已达上限：尝试本地缓存兜底
                print("[SlotSave] Max timeout reached (" .. LOAD_TIMEOUT_MAX .. "s), attempting local fallback")
                loadInProgress = false
                local cb = loadCallback
                loadCallback = nil
                ...
                if localData then
                    ...
                    SlotSaveSystem._FinalizeLoad(loadSlotId, localData, cb)
                    loadCurrentTimeout = LOAD_TIMEOUT_INIT -- 成功，重置给下次
                else
                    -- 无本地缓存，继续以最大超时重试
                    print("[SlotSave] No local cache, retrying with " .. LOAD_TIMEOUT_MAX .. "s timeout...")
                    SlotSaveSystem.LoadSlot(loadSlotId, function(success, isNewSlot)
                        if cb then cb(success, isNewSlot) end
                    end)
                end
