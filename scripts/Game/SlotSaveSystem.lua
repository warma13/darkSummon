...
local SHARD_FORMAT = 2         -- 分片格式版本号（headData 专用）
...
-- 云端连续失败保护
local consecutiveFailures = 0        -- 连续云端保存失败次数
local FAILURE_WARN_THRESHOLD = 3     -- 连续失败 N 次后显示持久横幅
local FAILURE_SUSPEND_THRESHOLD = 6  -- 连续失败 N 次后暂停自动保存
local saveSuspended = false          -- 自动保存是否已暂停

--- 云端保存成功时重置失败计数
local function OnCloudSaveOK()
    if consecutiveFailures > 0 then
        print("[SlotSave] Cloud save recovered after " .. consecutiveFailures .. " consecutive failures")
    end
    consecutiveFailures = 0
    saveSuspended = false
    -- 每次云端交互成功都重新校准防作弊时钟
    require("Game.ServerTime").Calibrate()
    healthy = true
    local okB, Banner = pcall(require, "Game.SaveWarningBanner")
    if okB and Banner then Banner.Hide() end
end

--- 云端保存失败时递增计数并检查阈值
local function OnCloudSaveFail()
    consecutiveFailures = consecutiveFailures + 1
    print("[SlotSave] Consecutive cloud failures: " .. consecutiveFailures)
    if consecutiveFailures >= FAILURE_WARN_THRESHOLD then
        local okB, Banner = pcall(require, "Game.SaveWarningBanner")
        if okB and Banner then
            Banner.Show("云端存档异常，请检查网络后刷新页面")
        end
    end
    if consecutiveFailures >= FAILURE_SUSPEND_THRESHOLD then
        saveSuspended = true
        healthy = false
        print("[SlotSave] Auto-save SUSPENDED due to " .. consecutiveFailures .. " consecutive failures")
    end
end
...
-- 加载超时保护（渐进式：首次5s，每次重试+1s，最大10s）
local LOAD_TIMEOUT_INIT = 5    -- 首次加载超时时间（秒）
local LOAD_TIMEOUT_MAX = 10    -- 最大超时时间（秒）

local loadCurrentTimeout = LOAD_TIMEOUT_INIT -- 当前超时时间
local loadStartTime = 0        -- 加载开始时间戳
local loadSlotId = 0           -- 正在加载的槽位
local loadCallback = nil       -- 加载回调（超时时使用）
local loadInProgress = false   -- 是否正在加载中
local loadGeneration = 0       -- 加载代次：每次新 LoadSlot 递增，旧回调通过比对丢弃
...
local initSeq = 0              -- 请求序号，防止过期回调干扰
local INIT_CLOUD_TIMEOUT = 3   -- clientCloud:Get 内部超时（秒）
local INIT_RETRY_INTERVALS = { 1, 1, 1 }  -- Init 专用重试间隔（快速降级）
local initCloudTimer = -1      -- 内部超时计时器
...
-- ============================================================================
-- 编码前数据消毒（防止 NaN/Inf/function 导致 cjson.encode 崩溃）
-- ============================================================================

--- 递归消毒 table，返回修复计数
--- 就地修改，不产生新 table
---@param t table
---@param path string       当前字段路径（用于日志定位）
---@param seen table|nil    循环引用检测集合
---@return number fixCount  修复的字段数
local function SanitizeTable(t, path, seen)
    seen = seen or {}
    if seen[t] then
        -- 循环引用：已经访问过，跳过
        return 0
    ...
    for k, v in pairs(t) do
        local vt = type(v)
        local fieldPath = path .. "." .. tostring(k)

        if vt == "number" then
            -- NaN: v ~= v；Inf: math.abs(v) == math.huge
            if v ~= v or v == math.huge or v == -math.huge then
                print("[SlotSave] SANITIZE: " .. fieldPath .. " = " .. tostring(v) .. " → 0")
                t[k] = 0
                fixes = fixes + 1
            end
        elseif vt == "function" or vt == "userdata" or vt == "thread" then
            print("[SlotSave] SANITIZE: " .. fieldPath .. " = <" .. vt .. "> → removed")
            t[k] = nil
            fixes = fixes + 1
        elseif vt == "table" then
            fixes = fixes + SanitizeTable(v, fieldPath, seen)
        end
        -- string / boolean / nil: 合法，不处理
    end
    return fixes
end
...
local function EncodeChunkedGroups(slotId, groups, saveSeq)
    ...
    for _, groupName in ipairs(GROUP_NAMES) do
        ...
        if groupData then
            -- 编码前消毒：修正 NaN/Inf/function 等非法值，防止 encode 崩溃
            local fixCount = SanitizeTable(groupData, groupName)
            if fixCount > 0 then
                print("[SlotSave] Sanitized " .. fixCount .. " field(s) in group " .. groupName)
            end
            ...
            if len <= CHUNK_SIZE then
                ...
                while pos <= len do
                    ...
                    -- UTF-8 安全：如果 endPos 落在多字节字符中间，向前回退到字符起始位置
                    if endPos < len then
                        while endPos >= pos do
                            local b = string.byte(jsonStr, endPos + 1)
                            -- 如果下一字节是 continuation byte (10xxxxxx)，说明 endPos 截断了多字节字符
...
local function SaveLocal(slotId, saveData)
    ...
    -- 写入 header: 字节长度:DJB2校验\n + JSON 正文
    local header = tostring(#jsonStr) .. ":" .. tostring(CalcChecksum(jsonStr)) .. "\n"
    local file = File(fileName, FILE_WRITE)
    if file and file:IsOpen() then
        file:WriteString(header .. jsonStr)
        file:Close()
    else
...
--- 从本地缓存读取（校验长度+DJB2，防止读到写半截的损坏文件）
---@param slotId number
---@return table|nil
local function LoadLocal(slotId)
    ...
    -- 尝试解析 header: "LEN:CHECKSUM\n"
    local nlPos = string.find(raw, "\n", 1, true)
    if nlPos then
        local header = string.sub(raw, 1, nlPos - 1)
        local lenStr, csStr = string.match(header, "^(%d+):(%d+)$")
        if lenStr and csStr then
            local expectedLen = tonumber(lenStr)
            local expectedCs = tonumber(csStr)
            local jsonStr = string.sub(raw, nlPos + 1)
            -- 校验长度
            if #jsonStr ~= expectedLen then
                print("[SlotSave] Local cache CORRUPT (len mismatch: expected=" .. expectedLen .. " actual=" .. #jsonStr .. "), discarding " .. fileName)
                return nil
            end
            -- 校验 DJB2
            if CalcChecksum(jsonStr) ~= expectedCs then
                print("[SlotSave] Local cache CORRUPT (checksum mismatch), discarding " .. fileName)
                return nil
            end
            local ok, data = pcall(cjson.decode, jsonStr)
            if ok then return data end
            print("[SlotSave] Local cache decode failed for " .. fileName)
            return nil
        end
    end

    -- 兼容旧格式（无 header，纯 JSON）
...
-- 版本迁移统一由 SaveRegistry 层处理（saveVersion 字段），此处不再重复
...
local function DoSave()
    ...
    batch:Save("slot_" .. activeSlot .. "_save", {
        ...
        error = function(code, reason)
            ...
            local okT, Toast = pcall(require, "Game.Toast")
            if okT and Toast and Toast.Show then
                Toast.Show("云端存档失败，正在重试…", {255, 100, 80})
            end
...
---@param _isInternalRetry? boolean  内部重试标记，外部不要传
function SlotSaveSystem.Init(onMetaReady, _isInternalRetry)
    initCallback = onMetaReady
    initState = "loading"
    if not _isInternalRetry then
        initRetryCount = 0  -- 外部调用重置重试计数
    end
    initSeq = initSeq + 1           -- 递增序号，使旧回调失效
    local mySeq = initSeq
    initCloudTimer = INIT_CLOUD_TIMEOUT  -- 启动内部超时计时器

    print("[SlotSave] Init: loading save_meta from cloud... (seq=" .. mySeq .. ", retry=" .. initRetryCount .. ")")

    clientCloud:Get("save_meta", {
        ok = function(values, iscores)
            if mySeq ~= initSeq then return end  -- 过期回调，丢弃
            initCloudTimer = -1                   -- 取消超时计时器
        ...
        error = function(code, reason)
            if mySeq ~= initSeq then return end  -- 过期回调，丢弃
            initCloudTimer = -1                   -- 取消超时计时器
            SlotSaveSystem._HandleInitError(reason)
        end,
    })
end

--- Init 云请求失败/超时的统一处理（内部方法）
---@param reason string|nil
function SlotSaveSystem._HandleInitError(reason)
    print("[SlotSave] Init: cloud read failed: " .. tostring(reason))
    initRetryCount = initRetryCount + 1
    if initRetryCount <= MAX_RETRY then
        local delay = INIT_RETRY_INTERVALS[initRetryCount] or 1
        ...
        if initCallback then
            initCallback(meta, true, "cloud_error")
        end
    end
end
...
function SlotSaveSystem.LoadSlot(slotId, onComplete)
    ...
    -- 递增加载代次：使上一次 LoadSlot 的所有飞行中回调失效
    loadGeneration = loadGeneration + 1
    local currentLoadGen = loadGeneration
    ...
    -- 包装回调：确保任何路径触发回调时都清除加载状态
    -- 增加 generation 检查：旧请求的回调到达时直接丢弃，避免干扰新请求
    local wrappedComplete = function(success, isNewSlot)
        if currentLoadGen ~= loadGeneration then
            print("[SlotSave] Stale load callback ignored (gen=" .. currentLoadGen .. " < current=" .. loadGeneration .. ")")
            return
        end
        loadInProgress = false
        loadCallback = nil
        if success then
            loadCurrentTimeout = LOAD_TIMEOUT_INIT -- 成功后重置超时
        end
        if onComplete then onComplete(success, isNewSlot) end
    end
    loadCallback = wrappedComplete
        ...
        ok = function(values, iscores)
            ...
            if head and type(head) == "table" and head.keys then
                ...
                if localSeq > 0 and cloudSeq < localSeq then
                    ...
                    if localData then
                        SlotSaveSystem._FinalizeLoad(slotId, localData, wrappedComplete)
                        return
                    end
                ...
                -- 分片格式：批量读取所有分组
                SlotSaveSystem._LoadShardedSlot(slotId, head, wrappedComplete)
            else
                -- 无 head 或格式不对，尝试旧格式回退
                print("[SlotSave] No valid head for slot " .. slotId .. ", trying fallback...")
                SlotSaveSystem._LoadFallback(slotId, wrappedComplete)
            end
        end,
        error = function(code, reason)
    ...
    batchGet:Fetch({
        ok = function(values, iscores)
            ...
            if not checksumOk then
                print("[SlotSave] Checksum failed, trying local cache first...")
                -- 校验失败：优先使用本地缓存（更可靠），而非损坏的云端数据
                local localData = LoadLocal(slotId)
                if localData then
                    print("[SlotSave] Checksum fallback: using local cache")
                    SlotSaveSystem._FinalizeLoad(slotId, localData, onComplete)
                elseif next(groups) then
                    -- 本地无缓存，退而求其次使用云端数据（可能部分损坏）
                    print("[SlotSave] WARNING: no local cache, accepting cloud data despite checksum failure")
...
function SlotSaveSystem._FinalizeLoad(slotId, saveData, onComplete)
    -- pcall 保护：反序列化可能因存档损坏而抛异常
    local ok, errMsg = pcall(function()
        -- saveSeqNo 回退检测（业务数据层序列号）
        local loadedSeqNo = saveData.saveSeqNo or 0
        if loadedSeqNo > 0 then
            print("[SlotSave] Loaded saveSeqNo=" .. loadedSeqNo .. " for slot " .. slotId)
        end
    ...
    if not ok then
        print("[SlotSave] !! _FinalizeLoad ERROR: " .. tostring(errMsg))
        healthy = false
        if onComplete then onComplete(false, false) end
        return
    end
    ...
    -- 校准防作弊时钟（云存档加载成功 = 刚完成网络通信，os.time() 可信）
    require("Game.ServerTime").Calibrate()
...
function SlotSaveSystem.MarkDirty()
    ...
    -- 如果正在云端保存中，同时标记 pendingSave，
    -- 确保云端回调完成后立即用最新数据再保存一次
    if savingInProgress then
        pendingSave = true
    end
end
...
function SlotSaveSystem.Update(dt)
    -- Init 云请求内部超时：回调一直没触发时，主动走 error 重试路径
    if initState == "loading" and initCloudTimer > 0 then
        initCloudTimer = initCloudTimer - dt
        if initCloudTimer <= 0 then
            initCloudTimer = -1
            initSeq = initSeq + 1  -- 使悬挂的回调失效
            print("[SlotSave] Init: cloud request no response after " .. INIT_CLOUD_TIMEOUT .. "s, treating as error")
            SlotSaveSystem._HandleInitError("timeout_no_callback")
        end
    end

    -- Init 重试计时（error 后等待 N 秒再重试）
    if initState == "loading" and initRetryTimer > 0 then
        initRetryTimer = initRetryTimer - dt
        if initRetryTimer <= 0 then
            initRetryTimer = -1
            SlotSaveSystem.Init(initCallback, true)  -- 内部重试，不重置计数
        end
        return
    end

    -- 加载超时保护：渐进式超时（5s → 6s → ... → 10s）
    -- 每次超时直接返回失败，让玩家手动重试；下次重试时等待时间更长
    if loadInProgress and loadStartTime > 0 then
        if os.time() - loadStartTime >= loadCurrentTimeout then
            print("[SlotSave] WARNING: LoadSlot timed out after " .. loadCurrentTimeout .. "s")

            local outerCallback = loadCallback
            loadInProgress = false
            loadCallback = nil

            -- 尝试本地缓存兜底
            local localData = LoadLocal(loadSlotId)
            if localData then
                print("[SlotSave] Timeout fallback: using local cache for slot " .. loadSlotId)
                loadCurrentTimeout = LOAD_TIMEOUT_INIT -- 成功，重置
                SlotSaveSystem._FinalizeLoad(loadSlotId, localData, outerCallback)
            else
                -- 无本地缓存：直接失败，返回选服界面让玩家手动重试
                -- 渐进增加下次超时时间（上限 LOAD_TIMEOUT_MAX）
                if loadCurrentTimeout < LOAD_TIMEOUT_MAX then
                    loadCurrentTimeout = loadCurrentTimeout + 1
                end
                print("[SlotSave] No local cache, load failed (next timeout=" .. loadCurrentTimeout .. "s)")
                if outerCallback then outerCallback(false, false) end
            end
        end
    end
    ...
    if savingInProgress and saveStartTime > 0 then
        if os.time() - saveStartTime >= CLOUD_SAVE_TIMEOUT then
            ...
            local okT, Toast = pcall(require, "Game.Toast")
            if okT and Toast and Toast.Show then
                Toast.Show("云端存档超时，正在重试…", {255, 180, 60})
            end
    ...
    -- 自动保存定时器（saveSuspended 时跳过，但计时器继续走，恢复后立即触发）
    ...
    while i <= #retryQueue do
        ...
        if now >= item.nextRetryTime then
            ...
            if item.retryCount > MAX_RETRY then
                ...
                local okT, Toast = pcall(require, "Game.Toast")
                if okT and Toast and Toast.Show then
                    Toast.Show("云端存档多次失败，请检查网络后重试", {255, 80, 80})
                end
            else
                -- 执行重试
                table.remove(retryQueue, i)
