...
-- 加载超时保护
local LOAD_TIMEOUT = 10        -- 云端加载超时时间（秒）
...
function SlotSaveSystem.Update(dt)
    ...
    -- 加载超时保护：云端回调未触发时，尝试本地缓存兜底
    if loadInProgress and loadStartTime > 0 then
        if os.time() - loadStartTime >= LOAD_TIMEOUT then
            print("[SlotSave] WARNING: LoadSlot timed out after " .. LOAD_TIMEOUT .. "s, attempting local fallback")
            loadInProgress = false
            ...
            if localData then
                ...
                -- 无本地缓存，通知上层失败
                print("[SlotSave] Timeout fallback: no local cache, load failed")
