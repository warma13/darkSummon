...
--- 憎恨之地难度等级列表（与 HatredLandData.DIFFICULTY_LEVELS 对应）
LB.HL_DIFF_LEVELS = { 0, 1, 3, 9 }

--- 获取憎恨之地某难度的每日排行榜 key
--- 格式：lb_hld0_20260423（难度0普通）、lb_hld1_20260423（难度1困难）等
---@param diffLevel number 难度等级 0/1/3/9
---@return string
function LB.GetHatredLandDiffDailyKey(diffLevel)
    return "lb_hld" .. diffLevel .. "_" .. TodayKey()
end
...
--- 上传憎恨之地按难度的每日排行榜（每个难度独立 key，原始伤害）
...
function LB.UploadHatredLandDiffDaily(rawDamage, difficultyLevel)
    ...
    clientCloud:SetInt(dailyKey, encoded, {
        ok = function()
            print("[LB] Hatred Land diff daily uploaded: damage=" .. rawDamage
                .. " diff=" .. tostring(difficultyLevel) .. " key=" .. dailyKey)
        end,
        error = function(code, reason)
            print("[LB] Hatred Land diff daily upload FAILED: " .. tostring(code) .. " " .. tostring(reason))
        end,
    })
end
